...
$(function() {
    ...
    // Setup
    $("input:not(.token-input):not(.assets-tagsinput > input), textarea, select").on("checkval change", function () {
    ...
    $(".tags-input").on('itemAdded itemRemoved change', function (e) {
        if($(this).parent().find('.assets-tagsinput').children().hasClass('label')) {
            $(this).parent().find('.assets-tagsinput').children('input[type=text]').attr('placeholder', '');
        }
        else {
            $(this).parent().find('.assets-tagsinput').children('input[type=text]').attr('placeholder', '- Bootstrap tags input');
        }
    }).trigger('change');
